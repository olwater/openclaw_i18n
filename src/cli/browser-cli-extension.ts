...
export async function installChromeExtension(opts?: {
  ...
  if (!hasManifest(src)) {
    throw new Error(t("Bundled Chrome extension is missing. Reinstall OpenClaw and try again."));
  }
  ...
  if (!hasManifest(dest)) {
    throw new Error(t("Chrome extension install failed (manifest.json missing). Try again."));
  }
...
export function registerBrowserExtensionCommands(
  ...
  const ext = browser.command("extension").description(t("Chrome extension helpers"));

  ext
    .command("install")
    .description(t("Install the Chrome extension to a stable local path"))
    .action(async (_opts, cmd) => {
      ...
      defaultRuntime.error(
        info(
          [
            copied ? t("Copied to clipboard.") : t("Copy to clipboard unavailable."),
            "Next:",
  ...
  ext
    .command("path")
    .description(t("Print the path to the installed Chrome extension (load unpacked)"))
    .action(async (_opts, cmd) => {
      const parent = parentOpts(cmd);
      ...
      if (!hasManifest(dir)) {
        defaultRuntime.error(
          danger(
            [
              `Chrome extension is not installed. Run: "${formatCliCommand(t("openclaw browser extension install"))}"`,
      ...
      if (copied) {
        defaultRuntime.error(info(t("Copied to clipboard.")));
      }
    });
}
