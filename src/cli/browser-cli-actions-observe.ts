...
export function registerBrowserActionObserveCommands(
  ...
  browser
    .command("console")
    .description(t("Get recent console messages"))
    .option("--level <level>", t("Filter by level (error, warn, info)"))
  ...
  browser
    .command("pdf")
    .description(t("Save page as PDF"))
    .option("--target-id <id>", t("CDP target id (or unique prefix)"))
    .action(async (opts, cmd) => {
      const parent = parentOpts(cmd);
      const profile = parent?.browserProfile;
  ...
  browser
    ...
    .description(t("Wait for a network response and return its body"))
    .argument("<url>", t("URL (exact, substring, or glob like **/api)"))
    .option("--target-id <id>", t("CDP target id (or unique prefix)"))
    .option(
      "--timeout-ms <ms>",
      t("How long to wait for the response (default: 20000)"),
      (v: string) => Number(v),
    )
    .option("--max-chars <n>", t("Max body chars to return (default: 200000)"), (v: string) =>
      Number(v),
    )
