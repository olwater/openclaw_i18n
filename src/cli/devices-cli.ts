...
function formatTokenSummary(tokens: DeviceTokenSummary[] | undefined) {
  ...
  const parts = tokens
    .map((tok) => `${tok.role}${tok.revokedAtMs ? t(" (revoked)") : ""}`)
...
export function registerDevicesCli(program: Command) {
  ...
  devicesCallOpts(
    devices
      ...
      .option("--latest", t("Approve the most recent pending request"), false)
      .action(async (requestId: string | undefined, opts: DevicesRpcOpts) => {
        ...
        if (!resolvedRequestId) {
          defaultRuntime.error(t("No pending device pairing requests to approve"));
          defaultRuntime.exit(1);
          return;
        }
