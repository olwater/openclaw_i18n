...
function buildSkillStatus(
  ...
  const { required, missing, requirementsSatisfied, configChecks } = requirementStatus;
  ...
  return {
    ...
    requirements: {
      bins: required.bins,
      anyBins: required.anyBins,
      env: required.env,
      config: required.config,
      os: required.os,
    },
    missing,
    configChecks: configChecks.map((check) => ({
      ...check,
      value: resolveConfigPath(config, check.path),
    })),
