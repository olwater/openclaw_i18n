  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        t("Stored in: ~/.openclaw/openclaw.json (gateway.auth.token) or OPENCLAW_GATEWAY_TOKEN."),
        t("Web UI stores a copy in this browser's localStorage (openclaw.control.settings.v1)."),
        `Get the tokenized link anytime: ${formatCliCommand(t("openclaw dashboard --no-open"))}`,
      ].join("\n"),
      t("Token"),
    );
    ...
    if (hatchChoice === "tui") {
      ...
      if (seededInBackground) {
        await prompter.note(
          t("Web UI seeded in the background. Open later with: {command}").replace(
            "{command}",
      ...
      await prompter.note(
        [
          t("Dashboard link (with token): {url}").replace("{url}", authedUrl),
          controlUiOpened
  ...
  await prompter.outro(
    controlUiOpened
      ...
      : seededInBackground
        ...
        : t("Onboarding complete. Use the tokenized dashboard link above to control OpenClaw."),
  );
}
