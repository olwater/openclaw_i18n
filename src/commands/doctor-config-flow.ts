    ...
    if (hits.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand(t("openclaw doctor --fix"))}" to auto-resolve @username entries to numeric IDs (requires a Telegram bot token).`,
        ].join("\n"),
        t("Doctor warnings"),
      );
    }
    ...
    if (discordHits.length > 0) {
      note(
        [
          ...
          `- Discord IDs must be strings; run "${formatCliCommand(t("openclaw doctor --fix"))}" to convert numeric IDs to quoted strings.`,
        ].join("\n"),
        t("Doctor warnings"),
      );
    }
    ...
    if (allowFromScan.changes.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand(t("openclaw doctor --fix"))}" to add missing allowFrom wildcards.`,
        ].join("\n"),
        t("Doctor warnings"),
      );
    }
