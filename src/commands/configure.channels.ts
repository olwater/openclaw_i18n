...
export async function removeChannelConfigWizard(
  ...
  while (true) {
    ...
    if (configured.length === 0) {
      note(
        [
          t("No channel config found in openclaw.json."),
          `Tip: \`${formatCliCommand(t("openclaw channels status"))}\` shows what is configured and enabled.`,
        ].join("\n"),
        t("Remove channel"),
      );
      return next;
    ...
    const channel = guardCancel(
      await select({
        message: t("Remove which channel config?"),
        options: [
          ...configured.map((meta) => ({
            value: meta.id,
            label: meta.label,
            hint: t("Deletes tokens + settings from config (credentials stay on disk)"),
          })),
    ...
    note(
      [
        ...
        t("Note: credentials/sessions on disk are unchanged."),
      ].join("\n"),
      t("Channel removed"),
    );
  }
