  ...
  const { extensionsDir, pluginDirs } = await listInstalledPluginDirs({
    ...
    onReadError: (err) => {
      findings.push({
        ...
        severity: "warn",
        title: t("Plugin extensions directory scan failed") as unknown as string,
        detail: t("Static code scan could not list extensions directory: {{error}}", {
          error: String(err),
  ...
  for (const pluginName of pluginDirs) {
    ...
    for (const entry of extensionEntries) {
      ...
      if (extensionUsesSkippedScannerPath(entry)) {
        findings.push({
          ...
          severity: "warn",
          title: t('Plugin "{{name}}" entry path is hidden or node_modules', { name: pluginName }),
          detail: t(
    ...
    if (escapedEntries.length > 0) {
      findings.push({
        ...
        severity: "critical",
        title: t('Plugin "{{name}}" has extension entry path traversal', { name: pluginName }),
        detail: t("Found extension entries that escape the plugin directory:\n{{entries}}", {
          entries: escapedEntries.map((entry) => `  - ${entry}`).join("\n"),
        }),
        remediation: t(
    ...
    const summary = await skillScanner
      ...
      .catch((err) => {
        findings.push({
          checkId: "plugins.code_safety.scan_failed",
          severity: "warn",
          title: t('Plugin "{{name}}" code scan failed', { name: pluginName }),
          detail: t("Static code scan could not complete: {{error}}", { error: String(err) }),
          remediation: t(
            "Check file permissions and plugin layout, then rerun `openclaw security audit --deep`.",
          ),
        });
    ...
    if (summary.critical > 0) {
      ...
      findings.push({
        ...
        severity: "critical",
        title: t('Plugin "{{name}}" contains dangerous code patterns', { name: pluginName }),
        detail: t(
          "Found {{count}} critical issue(s) in {{scanned}} scanned file(s):\n{{details}}",
          { count: String(summary.critical), scanned: String(summary.scannedFiles), details },
        ),
        remediation: t(
      ...
      findings.push({
        checkId: "plugins.code_safety",
        severity: "warn",
        title: t('Plugin "{{name}}" contains suspicious code patterns', { name: pluginName }),
        detail: t("Found {{count}} warning(s) in {{scanned}} scanned file(s):\n{{details}}", {
          count: String(summary.warn),
          scanned: String(summary.scannedFiles),
          details,
        }),
        remediation: t("Review the flagged code to ensure it is intentional and safe."),
      });
    }
  }
    ...
    for (const entry of entries) {
      ...
      const summary = await skillScanner.scanDirectoryWithSummary(skillDir).catch((err) => {
        findings.push({
          ...
          severity: "warn",
          title: t('Skill "{{name}}" code scan failed', { name: skillName }),
          detail: t("Static code scan could not complete for {{dir}}: {{error}}", {
            dir: skillDir,
            error: String(err),
          }),
          remediation: t(
      ...
      if (summary.critical > 0) {
        ...
        findings.push({
          ...
          severity: "critical",
          title: t('Skill "{{name}}" contains dangerous code patterns', { name: skillName }),
          detail: t(
            "Found {{count}} critical issue(s) in {{scanned}} scanned file(s) under {{dir}}:\n{{details}}",
            {
              count: String(summary.critical),
              scanned: String(summary.scannedFiles),
              dir: skillDir,
              details,
            },
          ...
          remediation: t(
            'Review the skill source code before use. If untrusted, remove "{{dir}}".',
            { dir: skillDir },
          ),
        });
      } else if (summary.warn > 0) {
        ...
        findings.push({
          checkId: "skills.code_safety",
          severity: "warn",
          title: t('Skill "{{name}}" contains suspicious code patterns', { name: skillName }),
          detail: t(
          ...
          remediation: t("Review flagged lines to ensure the behavior is intentional and safe."),
        });
      }
    }
  }
